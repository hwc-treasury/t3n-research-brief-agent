...
export async function fetchUrl(url: string): Promise<{ body: string; finalUrl: string }> {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
...
export async function fetchTopic(topic: string): Promise<{
  body: string;
  url: string;
  title: string;
}> {
  ...
  let parsed: { title?: string; extract?: string; content_urls?: { desktop?: { page?: string } } };
  try {
