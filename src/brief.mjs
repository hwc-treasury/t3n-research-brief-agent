...
export function extractTitle(html, fallback) {
  ...
  if (og && og[1]) return decode(og[1]).trim();
  const t = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
  if (t && t[1]) return stripHtml(t[1]).trim() || fallback;
  const h1 = html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
  if (h1 && h1[1]) return stripHtml(h1[1]).trim() || fallback;
  return fallback;
}
...
export function draftBriefFromText(opts) {
  ...
  const facts = takeUnique(
    [
      ...sentences.filter((s) => FACT_RE.test(s) && s !== title),
      ...sentences.filter((s) => s !== problem && s !== title).slice(0, 4),
    ],
    MAX_FACTS,
  );
  ...
  const ranked = sentences
    .filter((s) => s !== problem && ACTION_RE.test(s) && s !== title)
...
function actionScore(s) {
  const low = s.toLowerCase();
  if (low.includes("next step")) return 5;
  if (low.includes("recommend")) return 4;
  if (low.includes("assign")) return 3;
  if (low.includes("should")) return 2;
  if (low.includes("must")) return 1;
  return 0;
}
